...
| **RN-025** | Regressão administrativa de etapa: quem possui `opp.admin` (ex.: SUPER ADMIN) pode retornar uma oportunidade aberta a qualquer etapa anterior não-terminal, com justificativa obrigatória; a operação é auditada com ação própria (`regress`). O avanço permanece regido por RN-001/RN-002 para todos os perfis. | DB (trigger) |
